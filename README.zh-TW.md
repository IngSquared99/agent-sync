...
agsy 預設掃描來源裡的 `rules/`、`skills/`、`workflows/`、`hooks/`。既有的庫若一直使用別的名稱，可透過 `build.categories.<類別>.from` 直接接上、不必搬檔案——見[設定檔](https://ingsquared99.github.io/agent-sync/#/zh-TW/config)。
