...
> **兩端與通道**：檔案內容只存在於兩端——「**來源端**」＝ sources 裡的原始檔、「**產物端**」＝ `.agsy/` 裡的複本。掛載是連結，工具目錄（`.claude/` 等）裡開啟的檔案即為產物端。連結本身稱「**掛載連結**」，只是通道、不儲存內容；通道的各種狀況見[情境全覽 C-4-5](07-scenarios.md#c-4-5-掛載連結的情境)。
