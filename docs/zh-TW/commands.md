...
- **掛載**：逐連結狀態（正常／遺失／指錯或已斷／被佔用／孤兒），以及每個 merge 目標的狀態（同步／agsy 條目被改／條目不見／不是 JSON 物件）。
...
反安裝：確認後先把 agsy 的 hook 條目從 merge 目標移出（`settings.json` 是你的檔案：只刪 agsy 的條目，`hooks` 鍵空了就刪鍵，檔案是 agsy 建的且變空才刪檔），再移除掛載連結（含根目錄 `AGENTS.md` 與三個 `hooks.json` 連結）與整個產物目錄；**保留 `agsy.yaml`**。只刪 agsy 建立的東西——實體目錄與檔案會略過並回報；manifest 記錄的孤兒連結也一併清除（每條路徑都先驗證確實是指向產物的連結才動手）；連結移除後空掉的掛載目錄也會移除。之後 `agsy apply` 一次即可全部重建。
