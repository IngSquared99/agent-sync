...
**要改 `out` 的話，順序很重要**：先在**舊設定**下跑 `agsy clean`（把舊連結和舊產物清乾淨），再改 `agsy.yaml`、跑 `agsy apply`。順序反過來的話，舊產物目錄會殘留在磁碟上沒人清（agsy 只認得目前設定的 out）。改完也記得更新 `.gitignore`。

## `build.categories`：類別目錄對應
...
- 預設 `from` 和 `to` 同名，**什麼時候會改 `from`**：你的既有庫子目錄不叫標準名稱時（例如一直都叫 `prompts/`），改 `from: prompts` 就能不搬家直接接上——這是[來源命名規範](overview.md#來源目錄必須照規範命名)的逃生口。`to` 則幾乎不需要動。
...
## `build.on_conflict`：同名衝突策略（每類別必填）
...
內建適配器提供的掛載組合見[適配器說明](adapters.md)；不在內建清單的工具可以自己加一個 mount 項目，`init` 編輯模式會把自訂項目原樣保留。
