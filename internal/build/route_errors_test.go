...
// Target problems are collected per file so plan can list them all at once;
// unaffected workflows still resolve their tools, and Execute refuses to build.
func TestTargetProblemsAreCollectedNotFatal(t *testing.T) {
	...
	for _, it := range p.Items {
		if it.Category == "workflows" && it.Name == "deploy.md" && len(it.Tools) == 0 {
			t.Error("a broken file must not affect target resolution of other workflows")
		}
	}
