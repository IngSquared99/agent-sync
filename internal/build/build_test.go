...
build:
  out: .agsy
  on_conflict: {rules: %s, skills: %s, workflows: rename, hooks: error}
...
func TestFilterWorkflows(t *testing.T) {
	...
	wants := map[string]want{
		"deploy.md":       {tools: []string{"claude"}, skill: "deploy"},
		"standup.md":      {tools: []string{"antigravity"}, skill: ""}, // stub only
		"release-note.md": {tools: cfg.Build.Tools, skill: "release-note"},
	}
	for _, it := range p.Items {
		...
		if strings.Join(it.Tools, ",") != strings.Join(w.tools, ",") {
			t.Errorf("%s → tools %v, want %v", it.Name, it.Tools, w.tools)
		}
		if it.SkillName != w.skill {
			t.Errorf("%s → skill %q, want %q", it.Name, it.SkillName, w.skill)
		}
	}
}

func TestTargetUnknownToolFails(t *testing.T) {
	// A bad tool name must not abort Compute (that would hide the rest of
	// the plan); it is collected into RouteErrors and Execute refuses to build.
	cfg, lib, _ := setupTwoSources(t, "rename", "error")
	writeFile(t, filepath.Join(lib, "workflows", "bad.md"), "---\ntarget: [nosuch]\n---\nx\n")
	p := compute(t, cfg)
	if len(p.RouteErrors) != 1 || !strings.Contains(p.RouteErrors[0], "nosuch") {
		t.Errorf("RouteErrors = %v, want the unknown tool collected", p.RouteErrors)
	}
	if _, err := Execute(cfg, p); err == nil {
		t.Error("Execute must refuse while target errors exist")
	}
}
...
func TestExecuteEndToEnd(t *testing.T) {
	...
	// 2) a workflow without a target produces both forms: the derived skill
	// (full content) and the workflows stub (redirect for /name)
	for _, pth := range []string{
		filepath.Join("skills", "release-note", "SKILL.md"),
		filepath.Join("workflows", "release-note.md"),
	} {
		if _, err := os.Stat(filepath.Join(out, pth)); err != nil {
			t.Errorf("missing output %s", pth)
		}
	}
	// the derived skill front matter carries the mandatory fields
	raw, err := os.ReadFile(filepath.Join(out, "skills", "release-note", "SKILL.md"))
	if err != nil {
		t.Fatal(err)
	}
	for _, wantS := range []string{"name: release-note", "description:", "disable-model-invocation: true"} {
		if !strings.Contains(string(raw), wantS) {
			t.Errorf("derived skill is missing %q:\n%s", wantS, raw)
		}
	}
	// the stub names the skill it redirects to
	rawStub, err := os.ReadFile(filepath.Join(out, "workflows", "release-note.md"))
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(string(rawStub), "release-note") {
		t.Errorf("stub does not name its skill:\n%s", rawStub)
	}
	// a workflow targeting only claude gets no stub
	if _, err := os.Stat(filepath.Join(out, "workflows", "deploy.md")); err == nil {
		t.Error("deploy.md targets claude only; no stub should be produced")
	}
	// a workflow targeting only antigravity gets no derived skill
	if _, err := os.Stat(filepath.Join(out, "skills", "standup")); err == nil {
		t.Error("standup targets antigravity only; no derived skill should be produced")
	}

	// 3) the derived AGENTS.md concatenates every rule with markers
	rawA, err := os.ReadFile(filepath.Join(out, config.AgentsMD))
	if err != nil {
		t.Fatal(err)
	}
	for _, wantS := range []string{"<!-- agsy: rules/git-commit.md -->", "lib version", "flow version"} {
		if !strings.Contains(string(rawA), wantS) {
			t.Errorf("AGENTS.md is missing %q", wantS)
		}
	}

	// 4) every directory mount target must exist (otherwise links pointing there are broken)
	for _, mnt := range cfg.Mount {
		for _, sub := range mnt.Links {
			if sub == config.AgentsMD {
				if st, err := os.Stat(filepath.Join(out, sub)); err != nil || st.IsDir() {
					t.Errorf("derived %s missing", sub)
				}
				continue
			}
	...
	// 5) manifest: source baseline + per-output hashes + derived flags
	...
	if rn == nil || len(rn.Outs) != 2 {
		t.Fatalf("release-note should have two outputs (derived skill + stub), got %+v", rn)
	}
	for _, o := range rn.Outs {
		if !o.Derived || o.Hash == "" {
			t.Errorf("workflow outputs must be derived and hashed: %+v", o)
		}
	}
	if skill == nil || len(skill.Outs) != 1 || skill.Outs[0].Derived || skill.SrcHash == "" || len(skill.SrcFiles) != 2 {
		t.Fatalf("skill manifest data incomplete: %+v", skill)
	}

	// 6) rerunning the build must give the same result (full reproducibility is
...
// A workflow targeting only the stub tool has no derived skill; its
// workflows-directory form must then hold the full content verbatim, never a
// stub pointing at a skill that does not exist.
func TestStubOnlyWorkflowKeepsFullContent(t *testing.T) {
	cfg, lib, _ := setupTwoSources(t, "rename", "error")
	_ = lib
	p := compute(t, cfg)
	m, err := Execute(cfg, p)
	...
	raw, err := os.ReadFile(filepath.Join(cfg.OutDir(), "workflows", "standup.md"))
	if err != nil {
		t.Fatal(err)
	}
	// Full content, no redirect stub, and the agsy-specific target field
	// stripped from the front matter.
	if !strings.Contains(string(raw), "s\n") || strings.Contains(string(raw), "SKILL.md") {
		t.Errorf("stub-tool-only workflow must keep its full content, got:\n%s", raw)
	}
	if strings.Contains(string(raw), "target:") {
		t.Errorf("target field must be stripped from the workflows output, got:\n%s", raw)
	}
	for _, it := range m.Items {
		if it.Name == "standup.md" {
			if len(it.Outs) != 1 || !it.Outs[0].Derived {
				t.Errorf("a rewritten workflows form must be marked derived: %+v", it.Outs)
			}
		}
	}
}
