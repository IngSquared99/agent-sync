...
// linkFile on Windows: hard link (no privileges needed, unlike file symlinks
// which require Developer Mode). Hard links only work on the same volume; the
// mount dir and build.out both live inside the project, so that always holds.
func linkFile(absTarget, linkPath string) error {
	abs, err := filepath.Abs(absTarget)
	if err != nil {
		return err
	}
	if err := os.Link(abs, linkPath); err != nil {
		return fmt.Errorf("%w%s", err, i18n.T(" (hard link failed; the file system may not support hard links)"))
	}
	return nil
}
...
func isLink(fi os.FileInfo, path string) bool {
	...
	if fi.Mode().IsRegular() {
		// Hard links are ordinary files; identity checks handle them elsewhere.
		return false
	}
...
func platformHint() string {
	return i18n.T("\n(Windows note: agsy mounts directories as junctions and AGENTS.md as a hard link; neither needs privileges. If this still fails, make sure the target drive is NTFS)")
}
