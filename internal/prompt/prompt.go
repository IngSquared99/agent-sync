...
// AssumeYes is set by the global --yes / -y flag: every confirmation is
// treated as y, so apply / clean can run in CI and scripts.
var AssumeYes bool
...
func MultiSelect(msg string, options []string, preselected []int) []int {
	...
	if AssumeYes || !IsStdinTTY() {
		...
		// breaking the "needs confirmation → cancelled, never forced" rule.
		fmt.Println(i18n.T("  ✘ Non-interactive and --yes not given; treating as cancelled."))
		return nil
	}
	for {
